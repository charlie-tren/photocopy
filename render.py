...
<!-- Photocopy's own mark. Without these the browser falls back to
     charlietrenorden.com/favicon.ico and the tab wears the HUB's aperture icon.
     Relative paths, because the site is served from /photocopy/. The PNG and ICO
     keep transparent corners; apple-touch is flattened square, since iOS paints
     transparency black and applies its own mask. -->
...
<body>
  <div class="wrap">
    ...
    <div id="infobox" class="infobox" hidden>
      ...
      still uses.</p>
      <!-- No threshold is quoted here, because there isn't one. Measured
           26/08: pairs of images with nothing whatever to do with each other
           (18 portraits, 16 project cards) sat at a mean of 25.5 and 28.2, and
           frame 1 against that same unrelated set ran 24 to 44. So "unrelated"
           is a wide band overlapping the chain's own range, not a line at 32,
           and the earlier copy promised a verdict the number cannot give. -->
      <p><b>picture different</b> is how far this image is from frame 1 in
      structure. 0 is the same picture. Pictures with nothing in common usually
      land in the twenties or thirties.</p>
    </div>
